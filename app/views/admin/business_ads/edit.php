...
<form method="POST" action="<?= BASE_URL ?>/public<?= $adsBase ?>/edit/<?= $ad['id'] ?>" id="adEditForm">
<?= CSRF::field() ?>
<div class="row g-3" style="max-width:900px;margin:0 auto;padding:16px">
  ...
  <div class="col-12">
    ...
    <input type="text" name="business_name" class="af-input" required value="<?= Helper::e($ad['business_name']) ?>">
  </div>
  <div class="col-md-6">
    <label class="af-label">Contact Person</label>
    <input type="text" name="contact_person" class="af-input" value="<?= Helper::e($ad['contact_person']??'') ?>">
  </div>
  <div class="col-md-6">
    <label class="af-label">Mobile Number</label>
    <input type="tel" name="contact_phone" class="af-input" value="<?= Helper::e($ad['contact_phone']??'') ?>">
  </div>
  <div class="col-md-6">
    <label class="af-label">Email</label>
    <input type="email" name="contact_email" class="af-input" value="<?= Helper::e($ad['contact_email']??'') ?>">
  </div>
  <div class="col-md-6">
    <label class="af-label">Website</label>
    <input type="url" name="website_url" class="af-input" value="<?= Helper::e($ad['website_url']??'') ?>">
  </div>
  <div class="col-md-4">
    <label class="af-label">Facebook</label>
    <input type="url" name="facebook_url" class="af-input" value="<?= Helper::e($ad['facebook_url']??'') ?>">
  </div>
  <div class="col-md-4">
    <label class="af-label">Instagram</label>
    <input type="url" name="instagram_url" class="af-input" value="<?= Helper::e($ad['instagram_url']??'') ?>">
  </div>
  <div class="col-md-4">
    <label class="af-label">YouTube</label>
    <input type="url" name="youtube_url" class="af-input" value="<?= Helper::e($ad['youtube_url']??'') ?>">
  </div>
  <div class="col-12">
    <label class="af-label">Address</label>
    <textarea name="address" class="af-textarea" rows="2"><?= Helper::e($ad['address']??'') ?></textarea>
  </div>
  <div class="col-12">
    <label class="af-label">Short Description</label>
    <textarea name="small_desc" class="af-textarea" rows="2"><?= Helper::e($ad['small_desc']??'') ?></textarea>
  </div>
  ...
  <div class="col-md-6">
    ...
    <select name="package_id" class="af-select">
      ...
      <option value="<?= $p['id'] ?>" <?= ($ad['package_id']??'')==$p['id']?'selected':'' ?>>[<?= Helper::e($p['code']??'') ?>] <?= Helper::e($p['name']) ?> — ₹<?= number_format($p['price_inr'],0) ?></option>
      <?php endforeach; ?>
    </select>
  </div>
  <div class="col-md-3">
    <label class="af-label">Valid From</label>
    <input type="date" name="valid_from" class="af-input" value="<?= $ad['valid_from'] ?>">
  </div>
  <div class="col-md-3">
    <label class="af-label">Valid Until</label>
    <input type="date" name="valid_until" class="af-input" value="<?= $ad['valid_until'] ?>">
  </div>

  <div class="col-12"><div class="af-card-head mt-2">Payment & Status</div></div>
  <div class="col-md-3">
    <label class="af-label">Amount (₹)</label>
    <input type="number" name="payment_amount" class="af-input" step="0.01" value="<?= $ad['payment_amount']??'' ?>">
  </div>
  <div class="col-md-3">
    <label class="af-label">Payment Ref</label>
    <input type="text" name="payment_ref" class="af-input" value="<?= Helper::e($ad['payment_ref']??'') ?>">
  </div>
  <div class="col-md-3">
    ...
    <select name="payment_status" class="af-select">
      ...
      <option value="<?= $ps ?>" <?= $ad['payment_status']===$ps?'selected':'' ?>><?= ucfirst($ps) ?></option>
      <?php endforeach; ?>
    </select>
  </div>
  <div class="col-md-3">
    ...
    <select name="status" class="af-select">
      ...
      <option value="<?= $ss ?>" <?= $ad['status']===$ss?'selected':'' ?>><?= ucfirst($ss) ?></option>
      <?php endforeach; ?>
    </select>
  </div>
  <div class="col-12">
    <label class="af-label">Notes</label>
    <textarea name="notes" class="af-textarea" rows="2"><?= Helper::e($ad['notes']??'') ?></textarea>
  </div>
